...
public class TombolaService {
	...
	public synchronized List<PrizeInfo> getPrize(@NotNull Long prizeId) {
		...
		try {
			...
			AuditQuery query = reader.createQuery().forRevisionsOfEntity(Prize.class, true, true)
					.addProjection(AuditEntity.revisionNumber()).addProjection(AuditEntity.selectEntity(false))
					.add(AuditEntity.id().eq(prizeId)).addOrder(AuditEntity.revisionNumber().desc())
			...
			for (Object row : query.getResultList()) {
				...
				AuditQuery query2 = reader.createQuery().forRevisionsOfEntity(Ticket.class, true, true)
						.addProjection(AuditEntity.revisionType()).addProjection(AuditEntity.id())
				...
				if (ret.size() == 0) {
					ret.add(new PrizeInfo(prizeId, (String) entity.get("prizeName"), (Integer) entity.get("cnt"),
							(Integer) entity.get("issued"), new Date((long) entity.get("modifiedDate")),
							Arrays.asList()));
				}
			}
			LOG.debug("GetPrize username:'{}' ip:{} sessionId:{} ret.size:{}", userService.getUsername(),
					statusController.getClientIP(), statusController.getSessionId(), ret.size());
		} catch (Exception e) {
			LOG.error(e.getMessage(), e);
		}
		return ret;
	 ...
	 */
	protected PrizeInfo makePrizeInfo(Long prizeId, Long ticketId, Integer revisionNumber, RevisionType revisionType,
			Map<String, Object> entity) {
		if (entity != null) {
			PrizeInfo ret = new PrizeInfo(prizeId, (String) entity.get("prizeName"), (Integer) entity.get("cnt"),
					(Integer) entity.get("issued"), new Date((long) entity.get("modifiedDate")),
					Arrays.asList(ticketId, (long) revisionNumber, (long) revisionType.ordinal()));
			// LOG.debug("MakePrizeInfo username:'{}' ip:{} sessionId:{} ret:{}",
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#getPrizes},
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#getPrizesReadyToWin}
	 * 
	 * @return {@link List}&lt;{@link PrizeInfo}&gt;
	 */
	@Transactional(readOnly = true)
	...
	public synchronized List<TicketAudit> getTicket(@NotNull Long ticketId) {
		...
		try {
			...
			AuditQuery query = reader.createQuery().forRevisionsOfEntity(Ticket.class, true, true)
					.addProjection(AuditEntity.revisionNumber()).addProjection(AuditEntity.revisionType())
					.addProjection(AuditEntity.selectEntity(false)).add(AuditEntity.id().eq(ticketId))
					.addOrder(AuditEntity.revisionNumber().desc()).addOrder(AuditEntity.id().desc());
			for (Object row : query.getResultList()) {
				Object[] arr = (Object[]) row;
				Integer revisionNumber = (Integer) arr[0];
			...
			LOG.debug("GetTicket username:'{}' ip:{} sessionId:{} ret:{}", userService.getUsername(),
					statusController.getClientIP(), statusController.getSessionId(), ret);
		} catch (Exception e) {
			LOG.error(e.getMessage(), e);
		}
		return ret;
	 ...
	 */
	protected TicketAudit makeTicketAudit(Long ticketId, Integer revisionNumber, RevisionType revisionType,
			Map<String, Object> entity) {
		if (entity != null) {
			TicketAudit ret = new TicketAudit(ticketId, (entity.get("win") == null) ? 0 : (long) entity.get("win"),
					(entity.get("modifiedDate") == null) ? new Date(0) : new Date((long) entity.get("modifiedDate")),
					revisionNumber, revisionType);
			// LOG.debug("MakeTicketInfo username:'{}' ip:{} sessionId:{} ret:{}",
			// userService.getUsername(), statusController.getClientIP(),
			// statusController.getSessionId(), ret);
			return ret;
		} else {
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#getTicketsPage}
	 * and {@link local.intranet.tombola.api.controller.IndexController#getManager}
	 * 
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#getTicketsWinPage},
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#getTicketsPrizes}
	 * and {@link local.intranet.tombola.api.controller.IndexController#getManager}
	 * 
	...
	public synchronized List<TicketInfo> getTicketsPrizes(@NotNull Integer cnt) {
		...
		if (cnt > 0) {
			...
			for (Ticket l : ticketRepository.findPageAndReadyWin(PageRequest.of(0, (int) ticketRepository.count()))) {
				if (!(ticketInfo.add(new TicketInfo(l.getId(), l.getWin(), new Date(l.getModifiedDate()))))) {
					break;
				}
			}
				...
				try {
					for (Prize prize : prizeRepository.findByReadyToWin()) {
						if (cnt > 0) {
							...
							ret.add(new TicketInfo(ticketInfo.get(idx).getId(), prize.getId(),
									new Date(prize.getModifiedDate())));
	 ...
	 * {@link local.intranet.tombola.api.controller.IndexController#setManager}
	 * 
	 * @param {@link List}&lt;{@link Map.Entry}&lt;{@link Long},
	 *               {@link Integer}&gt;&gt;
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#patchPrizeName}
	 * 
	 * @param prizeId   {@link Long}
	...
	public synchronized PrizeInfo patchPrizeName(@NotNull Long prizeId, @NotNull String prizeName) {
		...
		if (prize.isEmpty()) {
			...
			try {
				...
				List<TicketInfo> list = p.getTicket().stream().map(
						ticket -> new TicketInfo(ticket.getId(), ticket.getWin(), new Date(ticket.getModifiedDate())))
						.collect(Collectors.toList());
				Collections.sort(list, new TicketInfoComparator());
				ret = new PrizeInfo(p.getId(), p.getPrizeName(), p.getCnt(), p.getIssued(), new Date(st),
						ticketInfoAsLong(list));
				LOG.info("PatchPrizeName username:'{}' {} prizeId:{} prizeName:'{}'", username, 0, prizeId, prizeName);
			} catch (Exception e) {
				LOG.error(e.getMessage(), e);
				throw e;
			}
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#patchPrizeTicket}
	 * and {@link local.intranet.tombola.api.controller.IndexController#setManager}
	 * 
	 * @param prizeId  {@link Long}
	 * @param ticketId {@link Long}
	 * @return {@link PrizeInfo}
	...
	public synchronized PrizeInfo patchPrizeTicket(@NotNull Long prizeId, @NotNull Long ticketId) {
		...
		if (!prize.isEmpty() && !ticket.isEmpty() && ticket.get().getWin() == 0) {
			...
			List<TicketInfo> list = v.getTicket().stream()
					...
					.collect(Collectors.toList());
			Collections.sort(list, new TicketInfoComparator());
			ret = new PrizeInfo(v.getId(), v.getPrizeName(), v.getCnt(), v.getIssued(), new Date(new Date().getTime()),
					ticketInfoAsLong(list));
			LOG.info("PatchPrizeTicket username:'{}' ticketId:{} prizeId:{} prizeName:'{}'", userService.getUsername(),
					ticketId, prizeId, prize.get().getPrizeName());
		} else {
			ret = new PrizeInfo();
		}
		return ret;
	...
	public synchronized void putPrizes(List<Map<String, Object>> prizes) {
		if (prizeRepository.count() == 0) {
			List<Map<String, Object>> value = prizes;
			if (value != null) {
			...
			LOG.info("PutPrizes username:{} cnt:{} {} '{}'", "admin", prizeRepository.count(), 0, "Create "
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#putTickets}
	 * 
	 * @param cnt {@link Integer}
	 */
	@Transactional
	 ...
	 * <p>
	 * For
	 * {@link local.intranet.tombola.api.controller.TombolaController#deleteTicket}
	 * 
	 * @param ticketId {@link Long}
	 */
	@Transactional
	public synchronized void deleteTicket(@NotNull Long ticketId) {
		// LOG.warn("DeleteTicket username:{} ticketId:{} prizeId:{} '{}'",
		// userService.getUsername(), ticketId, 0, "");
		...
		if (t.isEmpty()) {
			...
			LOG.info("DeleteTicket username:'{}' ticketId:{} prizeId:{} '{}'", userService.getUsername(), ticketId,
					prizeId, prizeName);
		}
	}
	 ...
	 * {@link local.intranet.tombola.api.controller.IndexController#getManager},
	 * {@link local.intranet.tombola.api.controller.IndexController#setManager},
	 * {@link #getPrizes}, {@link #getPrizesReadyToWin}, {@link #patchPrizeName} and

...
/**
 * 
 * {@link LoggingEventService} for {@link local.intranet.tombola.TombolaApplication}
 * <p>
 * An entity {@link LoggingEvent} without setters is only used to read data written by logback-spring DbAppender
 * 
 * @author Radek Kádner
 *
 */
...
public class LoggingEventService {
     ...
     * <p>
     * Used {@link local.intranet.tombola.api.model.repository.LoggingEventRepository#countTotalLoggingEvents}
     * 
     ...
     * <p>
     * Used {@link local.intranet.tombola.api.model.repository.LoggingEventRepository#findPageByLevelString}
     * 
     ...
     * <p>
     * Used {@link local.intranet.tombola.api.model.repository.LoggingEventRepository#findPageByCaller}
     * 
     * @param page int
     * @param cnt  int
     ...
     */
    @Transactional(readOnly = true)
    public Page<LoggingEventInfo> findPageByCaller(int page, @NotNull int cnt, Sort sort,
    		List<String> callerClass, List<String> callerMethod) {
    	try {
    		...
    		s = s.replaceAll(Direction.ASC.toString(), "").replaceAll(Direction.DESC.toString(), "")
    				.replaceAll(":", "");
    		s = s.trim();
    		Pageable pageable = PageRequest.of(page, cnt, JpaSort.unsafe(direction, s.split(" ,")));
    		Page<LoggingEvent> pa = loggingEventRepository.findPageByCaller(
    				pageable, callerClass, callerMethod, Arrays.asList(LOGGING_EVENT_INFO)); 
    		List<LoggingEventInfo> list = new ArrayList<>();
    		for (LoggingEvent l : pa) {
    			list.add(makeLoggingEventInfo(l)); 
    		}
	...
	protected LoggingEventInfo makeLoggingEventInfo(LoggingEvent loggingEvent) {
		...
		String arg0 = (loggingEvent.getArg0() == null) ?
				"[NULL]" : loggingEvent.getArg0();
		String arg1 = (loggingEvent.getArg1() == null) ?
				"[NULL]" : loggingEvent.getArg1();
		String arg2 = (loggingEvent.getArg2() == null) ?
				"[NULL]" : loggingEvent.getArg2();
		String arg3 = (loggingEvent.getArg3() == null) ?
				"[NULL]" : loggingEvent.getArg3();
		// LOG.debug("arg0:{} arg1:{} arg2:{} arg3:{}", arg0, arg1, arg2, arg3);
		LoggingEventInfo ret = new LoggingEventInfo(
				loggingEvent.getId(),
				loggingEvent.getFormattedMessage(), 
				loggingEvent.getLevelString(),
				(s.length > 0) ? s[s.length - 1] : "",
						loggingEvent.getCallerMethod(), arg0, arg1, arg2, arg3, new Date(loggingEvent.getTimestmp()));
		// LOG.debug("{}", ret);
		return ret;
	}
